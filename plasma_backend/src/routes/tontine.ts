 ...
 */
tontineRouter.get("/groups", async (req: Request, res: Response) => {
  const wallet = typeof req.query.wallet === "string" ? req.query.wallet.trim().toLowerCase() : null;
  if (wallet) {
    const result = await query(
      `SELECT g.id, g.name, g.type, g.payout_details, g.frequency_seconds, g.contribution_amount, g.collateral_amount, g.status, g.contract_tontine_id, g.smart_contract_address, g.created_at
  ...
  const result = await query(
    "SELECT id, name, type, payout_details, frequency_seconds, contribution_amount, collateral_amount, status, contract_tontine_id, smart_contract_address, created_at FROM tontine_groups ORDER BY created_at DESC"
  );
 ...
 */
tontineRouter.post("/groups/simple", async (req: Request, res: Response) => {
  ...
  const frequency_seconds =
    body.frequency_seconds != null && !Number.isNaN(Number(body.frequency_seconds))
  ...
  const collateral_amount_usdt =
    body.collateral_amount_usdt != null && !Number.isNaN(Number(body.collateral_amount_usdt))
  ...
  const memberSet = new Set<string>([creator]);
  for (const m of membersRaw) {
    const addr = typeof m === "string" ? m.trim().toLowerCase() : "";
    if (/^0x[a-fA-F0-9]{40}$/.test(addr)) memberSet.add(addr);
  }
  ...
  try {
    ...
    const groupResult = await client.query(
      `INSERT INTO tontine_groups (name, type, payout_details, frequency_seconds, contribution_amount, collateral_amount, status, contract_tontine_id, smart_contract_address)
       ...
       RETURNING id, name, type, payout_details, frequency_seconds, contribution_amount, collateral_amount, status, contract_tontine_id, smart_contract_address, created_at`,
      [name, frequency_seconds, contributionAmount.toString(), collateralAmount.toString()]
    );
    const group = groupResult.rows[0] as { id: string };
    const privateKey = generatePrivateKey();
    const account = privateKeyToAccount(privateKey);
    await client.query(
      "UPDATE tontine_groups SET deposit_wallet_address = $1, deposit_wallet_private_key = $2 WHERE id = $3",
      [account.address.toLowerCase(), privateKey, group.id]
    );
    for (let i = 0; i < membersOrdered.length; i++) {
      await client.query(
        `INSERT INTO tontine_members (tontine_group_id, wallet_address, turn_position, collateral_status)
         VALUES ($1, $2, $3, 'ok') ON CONFLICT (tontine_group_id, wallet_address) DO NOTHING`,
        [group.id, membersOrdered[i], i]
      );
    }
    ...
    const created = groupResult.rows[0] as Record<string, unknown>;
 ...
 */
tontineRouter.post("/groups", async (req: Request, res: Response) => {
  const {
    name,
    type: tontineType,
    service_provider,
  ...
  const type = (tontineType === "ESCROW_LINKED" ? "ESCROW_LINKED" : "STANDARD") as string;
  const payoutDetails =
    type === "ESCROW_LINKED" && service_provider
      ? JSON.stringify({
  ...
  try {
    ...
    const groupResult = await client.query(
      `INSERT INTO tontine_groups (name, type, payout_details, frequency_seconds, contribution_amount, collateral_amount, status, contract_tontine_id, smart_contract_address)
       ...
       RETURNING id, name, type, payout_details, frequency_seconds, contribution_amount, collateral_amount, status, contract_tontine_id, smart_contract_address, created_at`,
      [name ?? null, type, payoutDetails, frequency_seconds, contributionAmount.toString(), collateralAmount.toString(), contract_tontine_id, contractAddress]
    );
    const group = groupResult.rows[0] as { id: string };
    await client.query(
      `INSERT INTO tontine_members (tontine_group_id, wallet_address, turn_position, collateral_status)
 ...
 */
tontineRouter.post("/groups/from-tx", async (req: Request, res: Response) => {
  ...
  const contractAddress = smart_contract_address.toLowerCase() as `0x${string}`;
  const client = getChainClient();
  let tontineId: number | null = null;
  try {
    let receipt = await client.getTransactionReceipt({ hash: tx_hash as `0x${string}` });
    if (!receipt) {
    ...
    const TONTINE_CREATED_4 = parseAbiItem("event TontineCreated(uint256 indexed tontineId, uint256 contributionAmount, uint256 frequencySeconds, uint256 collateralAmount)");
    const TONTINE_CREATED_5 = parseAbiItem("event TontineCreated(uint256 indexed tontineId, uint256 contributionAmount, uint256 frequencySeconds, uint256 collateralAmount, address serviceProvider)");
    const logs = (receipt as { logs?: unknown[] }).logs ?? [];
    const blockNum = receipt.blockNumber ?? (block_number != null ? BigInt(block_number) : null);
    const txHashHex = tx_hash as `0x${string}`;
      ...
      for (const log of logs) {
        try {
          const decoded = decodeEventLog({
            ...
            topics: (log as { topics?: `0x${string}`[] }).topics as [`0x${string}`, ...`0x${string}`[]],
          });
    ...
    if (tontineId == null && blockNum != null) {
      ...
      const topic0 = getEventSelector(TONTINE_CREATED_4);
      try {
        ...
        if (ourLog?.args?.tontineId != null) tontineId = Number(ourLog.args.tontineId);
        ...
        if (tontineId == null) {
          const blockHex = numberToHex(blockNum);
          const rawLogs = (await client.request({
            method: "eth_getLogs",
            params: [{ topics: [topic0], fromBlock: blockHex, toBlock: blockHex }],
          })) as Array<{ transactionHash?: string; topics?: string[] }>;
          const raw = rawLogs.find((l) => l.transactionHash?.toLowerCase() === tx_hash.toLowerCase());
          if (raw?.topics?.[1]) tontineId = Number(BigInt(raw.topics[1]));
        }
        if (tontineId != null) console.log("[from-tx] Got tontineId from getLogs:", tontineId);
      } catch (e) {
...
async function createGroupFromTx(req: Request, res: Response): Promise<Response> {
  const tontineId = req.body.contract_tontine_id as number;
  if (tontineId == null) return res.status(400).json({ error: "contract_tontine_id required" });
  const {
    name,
    frequency_seconds,
    contribution_amount_usdt,
  ...
  const contributionAmount = BigInt(Math.round(Number(contribution_amount_usdt) * 10 ** USDT_DECIMALS));
  const collateralAmount = BigInt(Math.round(Number(collateral_amount_usdt) * 10 ** USDT_DECIMALS));
  const creator = (creator_wallet as string).trim().toLowerCase();
  const contractAddress = (smart_contract_address as string).trim().toLowerCase();
  try {
    await query("BEGIN");
    await query(
      `INSERT INTO users (wallet_address, reputation_score) VALUES ($1, 100) ON CONFLICT (wallet_address) DO NOTHING`,
    ...
    const groupResult = await query(
      `INSERT INTO tontine_groups (name, type, payout_details, frequency_seconds, contribution_amount, collateral_amount, status, contract_tontine_id, smart_contract_address)
       ...
       RETURNING id, name, type, payout_details, frequency_seconds, contribution_amount, collateral_amount, status, contract_tontine_id, smart_contract_address, created_at`,
      [name ?? null, frequency_seconds, contributionAmount.toString(), collateralAmount.toString(), tontineId, contractAddress]
    );
    const group = groupResult.rows[0] as { id: string };
    await query(
      `INSERT INTO tontine_members (tontine_group_id, wallet_address, turn_position, collateral_status)
    ...
    if (tx_hash && block_number != null) {
      await query(
        `INSERT INTO blockchain_events (tx_hash, block_number, method_name, from_address, to_address, payload)
         VALUES ($1, $2, 'TontineCreated', $3, $4, $5)`,
        [tx_hash, block_number, creator, contractAddress, JSON.stringify({ contract_tontine_id: tontineId, name: name ?? null })]
      );
    }
    await query("COMMIT");
    return res.status(201).json(groupResult.rows[0]);
 ...
 */
tontineRouter.post("/groups/register-member", async (req: Request, res: Response) => {
  const { contract_tontine_id, smart_contract_address, wallet } = req.body as {
    contract_tontine_id?: number;
    smart_contract_address?: string;
    wallet?: string;
  };
  ...
  try {
    const groupResult = await query(
      "SELECT id FROM tontine_groups WHERE contract_tontine_id = $1 AND LOWER(smart_contract_address) = $2",
      [contract_tontine_id, contractLower]
    );
    ...
    const countResult = await query(
      "SELECT COUNT(*)::int AS c FROM tontine_members WHERE tontine_group_id = $1",
      [groupId]
    );
    const turnPosition = (countResult.rows[0] as { c: number }).c;
    await query(
      "INSERT INTO users (wallet_address, reputation_score) VALUES ($1, 100) ON CONFLICT (wallet_address) DO NOTHING",
      [walletLower]
    );
    await query(
      "INSERT INTO tontine_members (tontine_group_id, wallet_address, turn_position, collateral_status) VALUES ($1, $2, $3, 'ok') ON CONFLICT (tontine_group_id, wallet_address) DO NOTHING",
      [groupId, walletLower, turnPosition]
    );
 ...
 */
tontineRouter.post("/escrow", async (req: Request, res: Response) => {
  const {
    name,
    service_provider,
    payout_description,
  ...
  try {
    ...
    const groupResult = await query(
      `INSERT INTO tontine_groups (name, type, payout_details, frequency_seconds, contribution_amount, collateral_amount, status, contract_tontine_id, smart_contract_address)
       ...
       RETURNING id, name, type, payout_details, frequency_seconds, contribution_amount, collateral_amount, status, contract_tontine_id, smart_contract_address, created_at`,
      [name ?? null, payoutDetails, frequency_seconds, contributionAmount.toString(), collateralAmount.toString(), contract_tontine_id, contractAddress]
    );
    const group = groupResult.rows[0] as { id: string };
    await query(
    ...
    if (tx_hash && block_number != null) {
      await query(
        "INSERT INTO blockchain_events (tx_hash, block_number, method_name, from_address, to_address, payload) VALUES ($1, $2, 'TontineCreated', $3, $4, $5)",
        [tx_hash, block_number, creator, contractAddress, JSON.stringify({ contract_tontine_id, name: name ?? null, type: "ESCROW_LINKED" })]
      );
    }
    await query("COMMIT");
    return res.status(201).json(groupResult.rows[0]);
 ...
 */
tontineRouter.get("/escrow-transactions", async (req: Request, res: Response) => {
  ...
  const result = await query(
    `SELECT e.id, e.tontine_group_id, e.contract_id, e.beneficiary, e.winner_address, e.amount, e.status, e.created_at, e.released_at
 ...
 */
tontineRouter.post("/escrow-transactions", async (req: Request, res: Response) => {
  const { tontine_group_id, contract_id, beneficiary, winner_address, amount } = req.body as {
  ...
  try {
    const result = await query(
      `INSERT INTO escrow_transactions (tontine_group_id, contract_id, beneficiary, winner_address, amount, status)
       VALUES ($1, $2, $3, $4, $5, 'LOCKED')
       RETURNING id, tontine_group_id, contract_id, beneficiary, winner_address, amount, status, created_at`,
      [tontine_group_id, contract_id ?? null, beneficiaryLower, winnerLower, amountBigInt.toString()]
    );
...
tontineRouter.get("/groups/:id", async (req: Request, res: Response) => {
  ...
  const row = groupResult.rows[0] as Record<string, unknown>;
 ...
 */
tontineRouter.post("/groups/:id/sign", async (req: Request, res: Response) => {
  const { id } = req.params;
  ...
  const memberRow = await query(
    "SELECT 1 FROM tontine_members WHERE tontine_group_id = $1 AND LOWER(wallet_address) = $2 AND turn_position = 0",
    [id, creator_wallet]
  );
  ...
  await query(
    "UPDATE tontine_groups SET creator_signed_at = NOW() WHERE id = $1",
    [id]
  );
  const groupResult = await query("SELECT * FROM tontine_groups WHERE id = $1", [id]);
 ...
 */
tontineRouter.post("/groups/:id/execute-turn", async (req: Request, res: Response) => {
  const { id } = req.params;
  const groupResult = await query(
    "SELECT id, current_turn_index, frequency_seconds, deposit_wallet_address, deposit_wallet_private_key FROM tontine_groups WHERE id = $1",
    [id]
  );
  if (groupResult.rows.length === 0) {
    return res.status(404).json({ error: "Group not found" });
  ...
  const row = groupResult.rows[0] as {
    ...
    deposit_wallet_private_key: string | null;
  };
  const membersResult = await query(
    "SELECT wallet_address FROM tontine_members WHERE tontine_group_id = $1 ORDER BY turn_position",
    [id]
  );
  ...
  const beneficiary = (membersResult.rows[current] as { wallet_address: string }).wallet_address;

  if (row.deposit_wallet_address && row.deposit_wallet_private_key && config.blockchain.usdtAddress) {
    ...
    if (balance > 0n) {
      ...
      try {
        const hash = await walletClient.writeContract({
          ...
          args: [beneficiary as `0x${string}`, balance],
        ...
        await query(
          `INSERT INTO tontine_payouts (tontine_group_id, tx_hash, block_number, to_address, amount)
           VALUES ($1, $2, $3, $4, $5)`,
          [id, hash, blockNumber.toString(), beneficiary.toLowerCase(), balance.toString()]
        );
        await query(
          `INSERT INTO blockchain_events (tx_hash, block_number, method_name, from_address, to_address, payload)
           VALUES ($1, $2, 'TontinePayout', $3, $4, $5)`,
          [hash, Number(blockNumber), row.deposit_wallet_address!.toLowerCase(), beneficiary.toLowerCase(), JSON.stringify({ tontine_group_id: id, amount: balance.toString() })]
        );
  ...
  await query(
    "UPDATE tontine_groups SET current_turn_index = $1, next_due_at = NOW() + ($2 || ' seconds')::interval WHERE id = $3",
    [nextIndex, String(frequency_seconds), id]
  );
  const updated = await query(
    "SELECT id, name, type, payout_details, frequency_seconds, contribution_amount, collateral_amount, status, contract_tontine_id, smart_contract_address, creator_signed_at, current_turn_index, next_due_at, created_at, deposit_wallet_address FROM tontine_groups WHERE id = $1",
    [id]
  );
 ...
 */
tontineRouter.get("/groups/:id/deposit-balance", async (req: Request, res: Response) => {
  const { id } = req.params;
  const groupResult = await query(
    "SELECT deposit_wallet_address FROM tontine_groups WHERE id = $1",
    [id]
  );
  if (groupResult.rows.length === 0) {
    return res.status(404).json({ error: "Group not found" });
  }
  const addr = (groupResult.rows[0] as { deposit_wallet_address: string | null }).deposit_wallet_address;
  if (!addr || !config.blockchain.usdtAddress) {
 ...
 */
tontineRouter.get("/groups/:id/payouts", async (req: Request, res: Response) => {
  const { id } = req.params;
  const result = await query(
    "SELECT id, tontine_group_id, tx_hash, block_number, to_address, amount, created_at FROM tontine_payouts WHERE tontine_group_id = $1 ORDER BY created_at DESC",
    [id]
  );
  res.json(result.rows);
});
