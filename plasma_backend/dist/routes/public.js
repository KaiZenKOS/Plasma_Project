 ...
 */
exports.publicRouter.get("/user/profile/:address?", async (req, res) => {
 ...
 */
exports.publicRouter.get("/history", async (req, res) => {
    ...
    if (address) {
        const result = await (0, client_js_1.query)("SELECT tx_hash, block_number, method_name, from_address, to_address, payload, created_at FROM blockchain_events WHERE from_address = $1 OR to_address = $1 ORDER BY block_number DESC, id DESC LIMIT $2", [address, limit]);
 ...
 */
exports.publicRouter.get("/history/backfill", async (req, res) => {
    const { tontineServiceAddress, rpcUrl, fromBlock: configFromBlock } = config_js_1.config.blockchain;
    ...
    try {
        ...
        const effectiveFrom = range > RPC_MAX_BLOCK_RANGE || (fromBlockConfig === 0 && range > defaultLookback)
            ? toBlock - BigInt(Math.min(RPC_MAX_BLOCK_RANGE, defaultLookback))
            : fromBlock;
        const eventNames = ["TontineCreated", "MemberJoined", "ContributionPaid", "CollateralSlashed"];
        const logArrays = await Promise.all(TONTINE_EVENTS.map((event, i) => client.getLogs({
    ...
    for (const log of logs) {
        ...
        if (wallet && fromAddress !== wallet)
            continue;
        const existing = await (0, client_js_1.query)("SELECT 1 FROM blockchain_events WHERE tx_hash = $1 AND block_number = $2 AND method_name = $3 LIMIT 1", [log.transactionHash, Number(log.blockNumber), methodName]);
        ...
        if (log.args) {
            try {
                serializedArgs = JSON.stringify(log.args, (key, value) => typeof value === "bigint" ? value.toString() : value);
            }
            catch (e) {
       ...
       VALUES ($1, $2, $3, $4, $5, $6)`, [
        ...
        if (methodName === "TontineCreated" && log.args) {
            ...
            if (contractTontineId != null) {
                const existingGroup = await (0, client_js_1.query)("SELECT id FROM tontine_groups WHERE contract_tontine_id = $1 AND LOWER(smart_contract_address) = $2 LIMIT 1", [contractTontineId, contractLower]);
                if (existingGroup.rows.length === 0) {
                    const client = await client_js_1.pool.connect();
                    try {
                        await client.query("BEGIN");
                        await client.query("INSERT INTO users (wallet_address, reputation_score) VALUES ($1, 100) ON CONFLICT (wallet_address) DO NOTHING", [fromAddress]);
               ...
               RETURNING id`, [frequencySeconds, contributionStr, collateralStr, contractTontineId, contractLower]);
                        const groupId = groupResult.rows[0].id;
                        await client.query("INSERT INTO tontine_members (tontine_group_id, wallet_address, turn_position, collateral_status) VALUES ($1, $2, 0, 'ok')", [groupId, fromAddress]);
